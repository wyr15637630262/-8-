<!--
  ...
  - @JsName onOffer
  - @Description
  - @DateTime 2017/12/28 16:08
  - @author 亚茹
  -->
<template>
  <div>
    <div class="onOffer_content">
      ...
      <table class="onOffer_table">
        ...
        <tbody v-if="offerProductList.length!==0">
        <tr v-for="(item,index) in offerProductList">
          <td>00{{index+1}}</td>
          ...
          <td>
            <a @click="offerEdit(item.product.id)">{{msg1}}</a>
            <a>|</a>
            <a @click="offerDelete(item.product.id)">{{msg2}}</a>
            <a>|</a>
            <a @click="outStock(item.product.id)">{{msg3}}</a>
          </td>
        </tr>
        </tbody>
        <tbody v-else-if="offerProductList.length===0">
        <tr>
          <td colspan="10" >暂无数据...</td>
        </tr>
        </tbody>
      </table>
      <div class="onOffer_page" v-if="offerProductList.length!==0">
        <div class="pages">
          <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      outStock: function (id) {
        var that = this
        rq.unShelvedProducts(id).then(function (data) {
          if (data.result === '成功') {
            rq.obtainProductList(that.productParams).then(function (data) {
              if (data.result === '成功') {
                console.log(data)
                that.productList = data.data.data
                that.offerProductList = []
                for (var i = 0; i < that.productList.length; i++) {
                  if (that.productList[i].product.productStatus === 1) {
                    that.offerProductList.push(that.productList[i])
                  }
                }
...
<!-- 引入 onOffer Scss 文件  -->
<style lang="scss" scoped>@import "scss/onOffer";</style>
